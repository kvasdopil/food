...
async function generateRecipe(options: CliOptions, apiKey: string): Promise<RecipeData> {
  ...
  try {
    ...
    throw new Error(`Failed to parse recipe JSON: ${(error as Error).message}\nRaw output:\n${jsonText}`);
  }
}

async function enhanceImagePrompt(recipe: RecipeData, basePrompt: string, apiKey: string) {
  const descriptiveIngredients = recipe.ingredients.slice(0, 6).map((item) => item.name).join(', ');
...
function parseArgs(argv: string[]): CliOptions {
  ...
  if (!positional.length) {
    throw new Error('Please supply a meal name (e.g. yarn ts-node scripts/recipe-generator.ts "Margherita Pizza").');
  }
...
async function main() {
  try {
    ...
    const cuisineTag =
      options.tags.find((tag) => !nonSettingTags.has(tag.toLowerCase())) ?? '';
    ...
    recipe.ingredients.forEach((ingredient) => {
      console.log(`- ${ingredient.amount} ${ingredient.name}${ingredient.notes ? ` (${ingredient.notes})` : ''}`);
    });
    console.log('\nInstructions:');
    ...
    console.error(
      `[recipe-generator] ${(error as Error).message}`,
    );
    process.exitCode = 1;
  }
}
