...
1. Copy the example environment file and set your Supabase credentials:
   ...
   - `SUPABASE_SERVICE_ROLE_KEY`: service role token (required for authentication verification, used by CLI scripts for storage + seeding)
   ...
   - `GEMINI_API_KEY` or `GOOGLE_API_KEY`: Google Gemini API key (for recipe text generation, refinement, and prompt enhancement - used by `/api/recipes/generate`, `/api/recipes/[slug]/refine`, `/api/recipes/[slug]/generate-image`, and `/api/images/generate-preview` endpoints). The API key is automatically loaded by library functions (`@/lib/gemini.ts` for text model calls and `@/lib/google-image-generation.ts` for image generation).
...
4. In the Vercel dashboard, add the following environment variables under **Settings → Environment Variables**, then redeploy:
   ...
   - `SUPABASE_SERVICE_ROLE_KEY` (required for authentication verification, storage, and database operations)
   - `EDIT_TOKEN` (required for protected API endpoints; also supports Supabase session tokens for logged-in users)
   - `GEMINI_API_KEY` or `GOOGLE_API_KEY` (required for recipe generation, refinement, and image prompt enhancement - automatically loaded by library functions)
...
#### Author Attribution & My Recipes

- **US-37**: As a logged-in user, I want my recipes to be attributed to me so others can see who created them. ✅ **Implemented**: When logged-in users create recipes, the creator's name and email are automatically saved to the `author_name` and `author_email` fields in the database. Recipes created via scripts (using `EDIT_TOKEN`) do not have author attribution.
- **US-38**: As a user, I want to see who created a recipe on the recipe page so I can know the source. ✅ **Implemented**: Recipe pages display "by {username}" after the description, showing the creator's display name (or email if no name is available). Scripted recipes (without author info) do not show attribution.
- **US-39**: As a logged-in user, I want to filter recipes to show only my own recipes so I can manage my contributions. ✅ **Implemented**: "My Recipes" option in the user menu filters recipes to show only those created by the current user. Uses a special "mine" tag in the URL (`/feed?tags=mine`) that is processed specially by the API (not stored in the database). The filter requires authentication and shows an empty state message if no recipes are found.
...
- **US-36**: As a logged-in user, I want to see an "Add Recipe" button on the feed page so I can create new recipes. ✅ **Implemented**: Add Recipe button appears to the left of the avatar for logged-in users, opens a unified recipe modal (fullscreen on mobile, centered window on desktop) that handles both creating new recipes and modifying existing recipes. The modal title dynamically changes between "Create recipe" and "Modify recipe" based on context. The flow: user enters meal description → AI generates recipe with name, tags, description, ingredients, and instructions → preview image is automatically generated → user can add recipe to database and navigate to the new recipe page. After a recipe is generated, users see two buttons: "Add recipe" (saves the recipe) and "Change something" (returns to modify mode with the generated recipe as the base, allowing iterative refinement before saving).
...
- **US-33**: As a user, I want to see recipe variations (e.g., vegetarian versions) so I can find alternative versions of recipes I like. ✅ **Implemented**: Meal variations are automatically detected and displayed on recipe pages. Recipes with the same base meal (e.g., "Fried Rice", "Tacos") are grouped together. Recipe pages show "This meal has N more versions" or "This meal has another version" with a horizontally scrollable view of version cards. Each card displays a small image with the recipe name overlaid, similar to feed cards. The current recipe is excluded from the versions list. Variations are stored using a normalized `variation_of` field in the database. Users can create custom variations by clicking the "Create version" button, which opens a unified recipe modal pre-filled with the current recipe. When creating a variant, if the original recipe doesn't have a `variation_of` value, the LLM automatically determines the normalized base meal name (e.g., "Burger" from "Grilled Halloumi Burger") and both recipes are updated to share the same `variation_of` value.
...
- **Author Attribution**: Recipes created by logged-in users automatically store the creator's name and email in `author_name` and `author_email` fields. Recipe pages display "by {username}" after the description. Scripted recipes (created via `EDIT_TOKEN`) do not have author attribution. Author fields are immutable after creation - they cannot be changed even if the recipe is edited by another user or script.
...
- **Shuffled feed order**: Recipes in the feed are shuffled using a random seed generated on the server during SSR. The feed page uses `export const dynamic = "force-dynamic"` to ensure dynamic rendering on each request, preventing Next.js from caching the server component. This ensures the seed is randomized on each page reload, providing fresh variety each time. The same seed persists during infinite scroll pagination, ensuring a consistent sequence while browsing. Shuffling is disabled when any filters are active (tags, favorites, or search), returning recipes in their original order instead.
...
- POST to `/api/recipes/generate` generates recipe content (ingredients, instructions, image prompts) using Gemini API from natural language user input. Does not save to database. Always returns the first generated variant (no refinement). Requires authentication via `EDIT_TOKEN` or Supabase session token, and `GEMINI_API_KEY` (or `GOOGLE_API_KEY`) configured on the server (automatically loaded by `@/lib/gemini.ts`). Accepts `userInput` parameter for parsing natural language descriptions.
- POST to `/api/images/generate-preview` generates preview images for recipes using Google AI Gemini Image API (`gemini-2.5-flash-image`). Stores images in Supabase Storage as `preview.{hash}.jpg` and returns the public URL. Never updates the database. Requires authentication via `EDIT_TOKEN` or Supabase session token, and `GEMINI_API_KEY` (or `GOOGLE_API_KEY`) configured on the server (automatically loaded by `@/lib/google-image-generation.ts`).
- POST to `/api/recipes/[slug]/refine` evaluates and refines an existing recipe in the database. Evaluates the recipe, and if issues are found, generates a refined version and updates it. Requires authentication via `EDIT_TOKEN` or Supabase session token, and `GEMINI_API_KEY` (or `GOOGLE_API_KEY`) configured on the server (automatically loaded by `@/lib/gemini.ts`).
- POST to `/api/recipes/[slug]/generate-image` generates and uploads images for existing recipes. Requires authentication via `EDIT_TOKEN` or Supabase session token, `FIREFLY_API_TOKEN`, and `GEMINI_API_KEY` (or `GOOGLE_API_KEY`) configured on the server (automatically loaded by `@/lib/gemini.ts` for prompt enrichment). Handles prompt enrichment, Firefly image generation, storage upload, and database update.
...
- Recipes are shuffled using a seed value provided by the frontend. The seed is generated randomly on the server during SSR and persists during pagination, ensuring a consistent sequence while browsing. Shuffling only occurs when no filters are active (tags, favorites, or search); when filters are applied, recipes are returned in their original order.
- Supports seed parameter: `?seed=N` → Shuffles recipes using the provided seed value (only when no filters are active)
...
- **Recipe Management**:
- ✅ Add new recipes via modal interface (with Supabase authentication) - **Implemented**: Add Recipe modal allows logged-in users to generate recipes from natural language descriptions. Uses a two-stage streaming approach: `/api/recipes/parse-user-input-stream` for parsing user input into structured metadata (title, description, tags), then `/api/recipes/generate-stream` for generating the full recipe (ingredients, instructions). Image generation is automatically triggered as soon as the meal description is available from the parsing stage. Uses `/api/images/generate-preview` for preview images and `/api/recipes` for saving to database. Supports Google OAuth authentication via Supabase. Recipe preview card appears immediately when parsing starts, updating progressively as fields stream in.

- ✅ Streaming recipe generation - **Implemented**: Two-stage streaming approach for optimal user experience:
  ...
  - ✅ Meal variations - **Implemented**: Recipes can be grouped by base meal name using the `variation_of` field. Recipe pages display a horizontally scrollable view showing other versions of the same meal (e.g., "This meal has 2 more versions"). Each version is displayed as a small card with an image and name overlaid, similar to feed cards. Users can navigate between versions by clicking on the version cards. The LLM automatically identifies and sets `variationOf` during recipe generation. A script (`scripts/populate-meal-variations.ts`) can analyze all recipes and populate `variation_of` values for existing recipes. **Custom variant creation**: Users can create custom variations by clicking the "Create version" button in the variations section. When creating a variant from a recipe that doesn't have a `variation_of` value, the LLM automatically determines the normalized base meal name (e.g., extracts "Burger" from "Grilled Halloumi Burger" and "Classic Beef Burger"). Both the original and new recipe are updated to share the same `variation_of` value, ensuring they appear in each other's variations lists. The prompt explicitly instructs the LLM to extract only the generic base meal type, removing descriptive adjectives, proteins, and specific ingredients.
...
- `src/app/feed/page.tsx`: server component that generates a random seed for SSR and passes it to the client component. Uses `export const dynamic = "force-dynamic"` to ensure the page is rendered dynamically on each request, preventing caching and ensuring a new random seed is generated on every page reload. Ensures the same seed is used for both server render and client hydration.
- `src/app/feed/feed-page-content.tsx`: client component displaying paginated recipe feed with infinite scroll. Reads search query and tags from URL. Accepts `initialSeed` prop from server component and passes it to the pagination hook. Responsive grid layout works on all screen sizes.
...
- `src/components/recipe-modal.tsx`: unified modal component using shadcn Dialog for creating and modifying recipes. Fullscreen on mobile, centered window on desktop. Handles both creating new recipes and modifying existing recipes (including creating variants). Modal title dynamically changes between "Create recipe" and "Modify recipe". When modifying, shows the base recipe card and allows users to specify what they want to change. The "Generate" or "Update" button is positioned inside the input box (always blue with magic wand icon). After recipe generation, shows two buttons inside the recipe card: "Add recipe" (saves the recipe) and "Change something" (returns to modify mode for iterative refinement). Generates recipes using two-stage streaming AI:
  - **Stage 1 (Parsing)**: Parses user input into structured metadata (title, description, tags) with progressive UI updates. Image generation starts automatically when description is received.
  - **Stage 2 (Generation)**: After parsing completes, automatically generates full recipe (ingredients, instructions) with progressive updates.
  - Shows recipe preview card immediately when parsing starts, updating progressively as fields arrive. Only enables "Add recipe" button after both stages complete. Saves recipes to database via `/api/recipes`. Uses shadcn Button and Alert components. Closes on Escape key or clicking outside. Uses Supabase authentication for API calls.
- `src/components/add-recipe-modal.tsx`: wrapper component that uses `RecipeModal` with `mode="create"` for backward compatibility.
- `src/components/create-variant-modal.tsx`: wrapper component that uses `RecipeModal` with `mode="modify"` and passes the base recipe for creating variants.
- `src/components/recipe-preview-card.tsx`: preview card component for displaying generated recipes during streaming. Supports partial data and shows loading states for missing fields. Displays full overlay with "Generating recipe..." when empty (during initial parsing), then shows a small "Streaming..." badge in the top-right corner once data appears, allowing users to see fields populate in real time. Uses `RecipeFeedCard` for rendering with fallback values for missing fields. Accepts an optional `actionButton` prop for displaying action buttons (like "Add recipe" and "Change something") below the card.
...
- `src/components/recipe/description.tsx`: description component that displays recipe description, author attribution ("by {username}"), clickable tag links (navigate to feed with tag filtered), favorite button, and meal versions scroll. Author attribution only shows for recipes with author information (scripted recipes don't show attribution). Tags use the "link" variant of TagChip for proper semantic HTML and navigation. Versions scroll appears after tags and shows "This meal has N more versions" or "This meal has another version" with a horizontally scrollable view of version cards. Includes a "Create version" button that appears in the variations list (or instead of the list if no variants exist). Clicking the button initiates login flow if user is not authenticated, or opens the unified recipe modal in modify mode with the current recipe pre-filled.
- `src/components/recipe/versions-scroll.tsx`: horizontally scrollable component for displaying and navigating between meal versions. Shows version count message as a small header and displays version cards in a scrollable row. Each card shows a small image (4:3 aspect ratio) with the recipe name overlaid on the image using a gradient overlay, similar to feed cards. Cards have shadows and hover effects. Clicking a version card navigates to that recipe. Includes a "Create version" button at the end of the scrollable list (dashed border, plus icon) that opens the unified recipe modal for creating a new variation. Displays even when no variants exist (shows only the create button).
...
- `src/app/api/recipes/route.ts`: API endpoint returning paginated recipes (20 per page) with pagination metadata. Recipes are shuffled using a seed value provided via `?seed=N` query parameter (only when no filters are active). When filters (tags, favorites, or search) are active, recipes are returned in their original order without shuffling. Supports `page` parameter for traditional pagination, `from={slug}` for slug-based pagination, `tags` parameter for filtering by tags (format: `tags=tag1+tag2`), and `q` or `search` parameter for searching by recipe name and tags.
- `src/lib/shuffle-utils.ts`: Utility functions for deterministic shuffling. Provides `seededShuffle()` for shuffling arrays with a seed value using Fisher-Yates algorithm.
- `src/hooks/usePaginatedRecipes.ts`: Hook for managing paginated recipe feed. Accepts `initialSeed` option to persist seed across pagination. Only includes seed in API requests when no filters are active (tags, favorites, or search).
...
- `src/app/api/recipes/parse-user-input-stream/route.ts`: Streaming API endpoint for parsing natural language user input into structured metadata. Uses `streamGemini()` from `@/lib/gemini.ts` and `PartialJsonParser` from `@/lib/partial-json-parser.ts` to stream NDJSON updates. Streams fields as they become available (title, description, tags, userComment, servings, cuisine). Returns NDJSON format with `Content-Type: application/x-ndjson`.

- `src/app/api/recipes/generate-stream/route.ts`: Streaming API endpoint for progressive recipe generation. Accepts structured input (title, description, tags) and uses `streamGemini()` from `@/lib/gemini.ts` and `PartialJsonParser` from `@/lib/partial-json-parser.ts` to stream NDJSON updates. Streams recipe fields as they become available (title, summary, tags, ingredients, instructions, prepTimeMinutes, cookTimeMinutes, servings). Returns NDJSON format with `Content-Type: application/x-ndjson`.
- `src/lib/gemini.ts`: Gemini API client functions including `callGemini()` for non-streaming calls and `streamGemini()` for streaming content using `streamGenerateContent` endpoint. `streamGemini()` properly handles NDJSON format by tracking brace depth to extract complete JSON objects (not splitting by newlines, which would break multi-line JSON). Handles authentication, error handling, and response parsing.

- `src/lib/google-image-generation.ts`: Google AI Gemini Image API client for generating images. Uses optimized prompts for high-resolution professional food photography. Automatically enhanced with styling guidance (professional restaurant quality, vibrant colors, sharp focus, natural lighting) and quality directives (ultra-high resolution, crisp details, vivid textures).
...
- `src/hooks/useRecipeGeneration.ts`: React hook for managing two-stage recipe generation with streaming support. `generateRecipe()` function:
  - **Stage 1**: Calls `/api/recipes/parse-user-input-stream` and reads streaming response using `response.body.getReader()`, parses NDJSON lines, and updates state incrementally as fields arrive (title, description, tags). Automatically triggers image generation when description field is received.
  - **Stage 2**: After parsing completes, automatically calls `/api/recipes/generate-stream` with the parsed data and streams recipe generation (ingredients, instructions, etc.).
  - Handles buffer management for incomplete lines, maps streamed fields to `GeneratedRecipe` structure, and generates slugs automatically. Tracks parsing and generation states separately (`isParsing`, `isGenerating`) to show appropriate UI states. "Add recipe" button is only enabled when both stages complete.
...
1. **Generate Recipe YAML** (using `/api/recipes/generate` endpoint):
   ...
   **Optional:** `--endpoint <url>` to specify API URL (defaults to `http://localhost:3000` or `RECIPE_API_URL` env var)
   ...
   The API endpoint requires `GEMINI_API_KEY` (or `GOOGLE_API_KEY`) to be configured on the server (Vercel environment variables for production). The API key is automatically loaded by `@/lib/gemini.ts`.
...
3. **Generate Recipe Image** (using API endpoint):
   ...
   **Optional:**
   ...
   The API endpoint requires `FIREFLY_API_TOKEN`, `GEMINI_API_KEY` (or `GOOGLE_API_KEY`), and optionally `FIREFLY_API_KEY` configured on the server. The Gemini API key is automatically loaded by `@/lib/gemini.ts` for prompt enrichment.
...
- **`scripts/refine-today-recipes.ts`**: Finds recipes created today, refines them, and updates their images
  - Queries the database directly using Supabase admin client to find recipes created today (UTC)
  - For each recipe, refines it using the `/api/recipes/[slug]/refine` endpoint
  - Updates the image using the `/api/recipes/[slug]/generate-image` endpoint
  - Processes recipes one by one with progress indicators
  - Requires `NEXT_PUBLIC_SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, and `EDIT_TOKEN` in `.env.local`
  - Usage:
    ```bash
    npx tsx scripts/refine-today-recipes.ts                    # Refine and update images
    ...
    npx tsx scripts/refine-today-recipes.ts --endpoint <url>  # Specify API endpoint
    npx tsx scripts/refine-today-recipes.ts --firefly-key <key> # Specify Firefly API key
    ```
  - The script handles errors gracefully and continues processing remaining recipes if one fails
...
- **`POST /api/recipes`**: Creates or updates a recipe in the database. Accepts normalized recipe payloads, upserting by slug. Supports `prepTimeMinutes`, `cookTimeMinutes`, and `variationOf` fields. The `variationOf` field stores a normalized base meal name for grouping meal variations (e.g., "Fried Rice", "Tacos", "Burger"). For logged-in users (authenticated via Supabase session token), automatically sets `author_name` and `author_email` from the authenticated user. For recipes created via `EDIT_TOKEN`, author fields are set to `null`. Author fields are immutable - existing recipes preserve their original author even when updated. When creating a variant (indicated by `originalRecipeSlug` in the payload), if the original recipe doesn't have a `variation_of` value, it is automatically updated to match the new recipe's `variation_of` value, ensuring both recipes share the same base meal name. Requires authentication via `Authorization: Bearer <EDIT_TOKEN>` or `Authorization: Bearer <SUPABASE_SESSION_TOKEN>` (for logged-in users). The server rejects unauthorized requests with `401`.
...
- **`POST /api/recipes/generate-stream`**: Streaming endpoint for progressive recipe generation. Accepts structured input (`title`, `description`, `tags`, `userComment`, `servings`, `cuisine`, `variationOf`, `isVariant`) and returns NDJSON stream with progressive field updates. Each line contains a JSON object with `type: "field"` (field updates), `type: "complete"` (generation finished), or `type: "error"` (error occurred). Fields are streamed as they become available (title, summary, tags, ingredients, instructions, prepTimeMinutes, cookTimeMinutes, servings, variationOf, etc.). When `variationOf` is already provided, it is excluded from the schema and prompt (the value is overridden after generation). When `isVariant` is true and `variationOf` is not provided, the LLM is instructed to determine the normalized base meal name (e.g., "Burger" from "Grilled Halloumi Burger"). The UI updates progressively as fields arrive. Requires authentication via `Authorization: Bearer <EDIT_TOKEN>` or `Authorization: Bearer <SUPABASE_SESSION_TOKEN>`, and `GEMINI_API_KEY` (or `GOOGLE_API_KEY`) configured on the server. Uses `streamGenerateContent` from Gemini API for real-time streaming.
...
- **`POST /api/images/generate-preview`**: Generates preview images for recipes using Google AI Gemini Image API (`gemini-2.5-flash-image` model). Accepts `description` parameter (meal description only). Uses an optimized prompt for high-resolution professional food photography with vibrant colors, sharp focus, natural lighting, and commercial food photography aesthetic. Stores images in Supabase Storage as `preview.{hash}.jpg`. Returns the public URL of the stored image. Never updates the database. Automatically triggered during recipe generation as soon as the meal description is available from the parse-user-input-stream endpoint. Requires authentication via `Authorization: Bearer <EDIT_TOKEN>` or `Authorization: Bearer <SUPABASE_SESSION_TOKEN>`, and `GEMINI_API_KEY` (or `GOOGLE_API_KEY`) configured on the server (automatically loaded by `@/lib/google-image-generation.ts`).
- **`POST /api/recipes/[slug]/refine`**: Evaluates and refines an existing recipe in the database. Evaluates the recipe against quality standards (including title and description rules), and if issues are found, generates a refined version and updates the database. Returns evaluation results and the updated recipe. Preserves original `author_name` and `author_email` fields during refinement (author fields are immutable). Requires authentication via `Authorization: Bearer <EDIT_TOKEN>` or `Authorization: Bearer <SUPABASE_SESSION_TOKEN>`, and `GEMINI_API_KEY` (or `GOOGLE_API_KEY`) configured on the server (automatically loaded by `@/lib/gemini.ts`). The evaluation checks for formatting issues, metric measurements, ingredient formatting, and enforces the same title and description rules as recipe generation (no subjective descriptors in titles, objective descriptions only).
- **`POST /api/recipes/[slug]/generate-image`**: Generates and uploads an image for an existing recipe. Loads recipe from database, enriches prompt using Gemini API, generates image with Firefly API, uploads to Supabase Storage, and updates recipe's `image_url` in database. Requires authentication via `Authorization: Bearer <EDIT_TOKEN>` or `Authorization: Bearer <SUPABASE_SESSION_TOKEN>`, `FIREFLY_API_TOKEN`, and `GEMINI_API_KEY` (or `GOOGLE_API_KEY`) configured on the server (automatically loaded by `@/lib/gemini.ts` for prompt enrichment). Optional `x-firefly-key` header for Firefly API key (defaults to `FIREFLY_API_KEY` env var or "clio-playground-web").
...
5. **Next Recipe Navigation**:
   ...
   - Note: Feed order is shuffled using a seed that persists during pagination but randomizes on page reload
...
- `POST /api/recipes/generate` - Recipe generation (non-streaming)
- `POST /api/recipes/parse-user-input-stream` - Streaming user input parsing
- `POST /api/recipes/generate-stream` - Streaming recipe generation
...
- `POST /api/images/generate-preview` - Preview image generation (automatically triggered during recipe generation)
