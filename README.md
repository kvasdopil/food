...
- `GET /api/recipes` - Recipe feed listing. Supports `?tags=tag1+tag2` for tag filtering (special "mine" tag filters by author_email), `?q=searchterm` for searching, `?favorites=true` for filtering favorites, and `?page=N` or `?from={slug}` for pagination.
