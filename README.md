...
- `src/components/add-recipe-modal.tsx`: responsive modal component using shadcn Dialog for adding recipes. Fullscreen on mobile, centered window on desktop. Allows users to enter natural language descriptions of meals, generates recipes using two-stage streaming AI:
...
- **`POST /api/recipes/generate-stream`**: Streaming endpoint for progressive recipe generation. Accepts structured input (`title`, `description`, `tags`, `userComment`, `servings`, `cuisine`) and returns NDJSON stream with progressive field updates. Each line contains a JSON object with `type: "field"` (field updates), `type: "complete"` (generation finished), or `type: "error"` (error occurred). Fields are streamed as they become available (title, summary, tags, ingredients, instructions, prepTimeMinutes, cookTimeMinutes, servings, etc.). The UI updates progressively as fields arrive. Requires authentication via `Authorization: Bearer <EDIT_TOKEN>` or `Authorization: Bearer <SUPABASE_SESSION_TOKEN>`, and `GEMINI_API_KEY` (or `GOOGLE_API_KEY`) configured on the server. Uses `streamGenerateContent` from Gemini API for real-time streaming.
